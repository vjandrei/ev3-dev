...
_DEFAULT_MANUFACTURER_NAME = 'EV3'
...
class AlexaGadget:
    ...
    def __init__(self, gadget_config_path=None):
        ...
        if not self.endpoint_id:
            self.endpoint_id = ('EV3' + self.radio_address)[:16]
    ...
    def main(self):
        ...
        if not args.clear or (args.clear and args.pair):
            ...
            if not self.is_paired():
                ...
                logger.info('Now in pairing mode.  You can pair ' + self.friendly_name + ' to your Echo device using the screen on the Echo device if it has one, or the Alexa app if it DOES NOT. See the Getting Started Guide for more details.')
